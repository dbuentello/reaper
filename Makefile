...
# reaper Server & Agent version number
VERSION=$(shell cat pkg/reaper.go |grep "const Version ="|cut -d"\"" -f2)

MSERVER=reaperServer
MAGENT=reaperAgent
PASSWORD=reaper
BUILD=$(shell git rev-parse HEAD)
DIR=data/temp/v${VERSION}/${BUILD}
BIN=data/bin/
XBUILD=-X main.build=${BUILD} -X github.com/infosechoudini/reaper/pkg/agent.build=${BUILD}
...
server-windows:
	export GOOS=windows;export GOARCH=amd64;go build ${LDFLAGS} -o ${DIR}/${MSERVER}-${W}.exe cmd/reaperserver/main.go
...
server-linux:
	export GOOS=linux;export GOARCH=amd64;go build ${LDFLAGS} -o ${DIR}/${MSERVER}-${L} cmd/reaperserver/main.go

# Compile Agent - Linux mips
agent-mips:
	export GOOS=linux;export GOARCH=mips;go build ${LDFLAGS} ${GCFLAGS} ${ASMFLAGS} -o ${DIR}/${MAGENT}-${M} cmd/reaperagent/main.go

# Compile Agent - Linux arm
agent-arm:
	export GOOS=linux;export GOARCH=arm;export GOARM=7;go build ${LDFLAGS} ${GCFLAGS} ${ASMFLAGS} -o ${DIR}/${MAGENT}-${A} cmd/reaperagent/main.go

# Compile Agent - Linux x64
agent-linux:
	export GOOS=linux;export GOARCH=amd64;go build ${LDFLAGS} ${GCFLAGS} ${ASMFLAGS} -o ${DIR}/${MAGENT}-${L} cmd/reaperagent/main.go
...
server-darwin:
	export GOOS=darwin;export GOARCH=amd64;go build ${LDFLAGS} -o ${DIR}/${MSERVER}-${D} cmd/reaperserver/main.go

# Compile Agent - Darwin x64
agent-darwin:
	export GOOS=darwin;export GOARCH=amd64;go build ${LDFLAGS} ${GCFLAGS} ${ASMFLAGS} -o ${DIR}/${MAGENT}-${D} cmd/reaperagent/main.go
...
package-agent-dll:
	...
	cd ${DIR};${PACKAGE} ${MAGENT}-DLL-v${VERSION}.7z reaper.dll
	cp ${DIR}/reaper.dll ${BIN}dll
