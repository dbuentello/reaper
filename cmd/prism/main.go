...
func main() {
	...
	message("info", fmt.Sprintf("Connecting to %s checking for reaper server version v0.7.0.BETA or earlier", url))
	err = sendPre8Message(a)
	if err != nil {
		if verbose {
			message("warn", err.Error())
		}
		message("note", fmt.Sprintf("%s is not a reaper server", url))
	} else {
		os.Exit(0)
	}

	// Send message to server and see if I get a RegInit message that can be decrypted
	message("info", fmt.Sprintf("Connecting to %s checking for reaper server version v0.8.0.BETA or greater", url))
	err = opaqueRegister(a)
	if err != nil {
		if verbose {
			message("warn", err.Error())
		}
		message("note", fmt.Sprintf("%s is not a reaper server using \"%s\" as a pre-shared key", url, psk))
	}
}
...
func opaqueRegister(a agent.Agent) error {
	...
	message("success", fmt.Sprintf("Verified reaper server v0.8.0.BETA or greater instance at %s", url))

	if verbose {
		message("note", fmt.Sprintf("Decrypted reaper message:\r\n%+v", regInitResp))
	}
...
func sendPre8Message(a agent.Agent) error {
	...
	if j.Type == "AgentControl" {
		message("success", fmt.Sprintf("Verified reaper server v0.7.0.BETA or earlier instance at %s", url))
		if verbose {
			message("note", fmt.Sprintf("reaper message:\r\n%+v", j))
		}
	} else {
...
func sendMessage(method string, m messages.Base, client *http.Client) (messages.Base, error) {
	...
	case "post":
		...
		if req != nil {
			...
			req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", reaperJWT))
		}
