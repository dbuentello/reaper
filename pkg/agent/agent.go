...
// GLOBAL VARIABLES
var build = "nonRelease" // build is the build number of the reaper Agent program set at compile time
...
type Agent struct {
	...
	Version       string          // Version is the version number of the reaper Agent program
	Build         string          // Build is the build number of the reaper Agent program
...
func (a *Agent) Run() error {
	...
	if a.Verbose {
		message("note", fmt.Sprintf("Agent version: %s", reaper.Version))
...
func getClient(protocol string, proxyURL string) (*http.Client, error) {
	...
	TLSConfig := &tls.Config{
		...
		InsecureSkipVerify: true, // #nosec G402 - see https://github.com/Ne0nd0g/reaper/issues/59 TODO fix this
		CipherSuites: []uint16{
