...
func OPAQUERegistrationInit(m messages.Base, opaqueServerKey kyber.Scalar) (messages.Base, error) {
	...
	if !bytes.Equal(userRegInit.UserID, m.ID.Bytes()) {
		if core.Verbose {
			...
			message("note", fmt.Sprintf("reaper Message UserID: %v", m.ID.Bytes()))
		}
		return returnMessage, errors.New("the OPAQUE UserID doesn't match the reaper message ID")
	}
...
func OPAQUERegistrationComplete(m messages.Base) (messages.Base, error) {
	...
	// Check to make sure reaper  UserID matches OPAQUE UserID
	if !bytes.Equal(m.ID.Bytes(), Agents[m.ID].OPAQUERecord.UserID) {
		return returnMessage, fmt.Errorf("the OPAQUE UserID: %v doesn't match the reaper UserID: %v", Agents[m.ID].OPAQUERecord.UserID, m.ID.Bytes())
	}
