...
import (
	...
	"github.com/infosechoudini/reaper/pkg/modules/shellcode"
)
		...
		if len(cmd) > 0 {
			...
			case "main":
				...
				case "use":
					menuUse(cmd[1:])
				case "version":
					color.Blue(fmt.Sprintf("reaper version: %s", reaper.Version))
				case "":
				default:
...
func menuSetAgent(agentID uuid.UUID) {
	for k := range agents.Agents {
		if agentID == agents.Agents[k].ID {
			...
			prompt.SetPrompt("\033[31mreaper[\033[32magent\033[31m][\033[33m" + shellAgent.String() + "\033[31m]»\033[0m ")
...
func menuSetModule(cmd string) {
	if len(cmd) > 0 {
		...
		if errModule != nil {
			...
			prompt.SetPrompt("\033[31mreaper[\033[32mmodule\033[31m][\033[33m" + shellModule.Name + "\033[31m]»\033[0m ")
...
func menuSetMain() {
	...
	prompt.SetPrompt("\033[31mreaper»\033[0m ")
...
func menuHelpMain() {
	color.Yellow("reaper C2 Server (version %s)", reaper.Version)
	table := tablewriter.NewWriter(os.Stdout)
	...
	data := [][]string{
		...
		{"banner", "Print the reaper banner", ""},
		{"exit", "Exit and close the reaper server", ""},
		{"interact", "Interact with an agent. Alias for Empire users", ""},
		{"quit", "Exit and close the reaper server", ""},
		{"remove", "Remove or delete a DEAD agent from the server"},
		{"sessions", "List all agents session information. Alias for MSF users", ""},
		{"use", "Use a function of reaper", "module"},
		{"version", "Print the reaper server version", ""},
	...
	table.Render()
	fmt.Println()
	message("info", "Visit the wiki for additional information https://github.com/Ne0nd0g/reaper/wiki/reaper-Server-Main-Menu")
}
...
func menuHelpAgent() {
	...
	message("info", "Visit the wiki for additional information "+
		"https://github.com/Ne0nd0g/reaper/wiki/reaper-Server-Agent-Menu")
}
...
func exit() {
	...
	if confirm(response) {
		color.Red("[!]Quitting")
		logging.Server("Shutting down reaper Server due to user input")
		os.Exit(0)
	}
}
